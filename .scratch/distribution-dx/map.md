...
- [Reserve the npm names](issues/03-reserve-npm-names.md) (2026-08-28): `zazz-ui@0.0.0` placeholder is live on npm; `@zazz-ui` scope controlled via the `zazz-ui` org, owned by Derek's `thederek` account; kit stays unpublished; `pnpm dlx zazz-ui@latest` is safe to assume.
