...
- 19: `packages/core/scripts/build-dist.mjs` owns all `dist/*.css` (`lightningcss.bundleAsync`, minify, no targets, in-memory entry per output); exported `DIST_CSS` constant is the map (layers, base, utilities-core, 8 families, utilities, 37 primitives, zazz) and is checked against `index.css`'s plain `@import` list at build time; every modular file is prefixed with the bundled layer order. `vp pack` emits only `zazz.js`; build = `tsc → pack → build-dist → heads → sri`; `lightningcss ^1.33.0` in the catalog. `zazz.css` 338,792 B vs 338,793 B before; `@container style()`/`@property`/`light-dark()` counts unchanged (65/297/28); `sri.json` 132 files. Caveat: lightningcss folds the six-name `@layer` statement (order verified equivalent). ([19](issues/19-build-dist.md))
...
- 29: docs — `core-concepts/style-props.mdx` (new "Style props vs classes": value-shape rule, 44 props by family, step spacing, suffixes, coordination, attribute gate, pitfalls, CSP pointer) and `core-concepts/responsive-design.mdx` rewritten as "Responsive: container vs viewport" (`--bp-*` nearest-container vs `--screen-*` viewport, the parent-resolves-`style()` nuance from ticket 09, prefixes vs suffixes, the 0.5 shift → upgrading page); `meta.json` and the template link updated. Forward links to `csp`/`upgrading` resolve once tickets 30/31 land. `foundation/utilities/responsive-design.mdx` is stale (base-only spacing, "body sets the flags") — ticket 32. ([29](issues/29-docs-style-props-responsive.md))
- 34: no regression vs 0.4.1 — `src/examples-migrated.test.ts` sweeps `examples/*.html` + `src/primitives/**/*.html` for the 28 *retired* rule names of `migrations/0.5.0.json` (chain-shifted names such as `data-container=sm` are live 0.5 names and cannot be judged textually; `--gap-sm…xl` only as `var()` reads since they are the `--gap` prop's forms); `regression.md` compares computed styles of layout/responsive/products at 600/900/1300 against a `aff8707` worktree: every resolved value identical, the only differences the renamed band line names (`container-xs-*` → `container-sm-*` …) and ticket 28's style-props block. Probe: `probe-computed.mjs`. ([34](issues/34-regression-check.md))
- 35: release-ready. 0.5.0 column filled in `CHANGELOG.md` (338,886 B raw, 31,088 br, 42,035 gzip, 2,767 rules, `_utilities.css` 160,490 B, CDN n/a until published, coverage 60,481 / 278,405) with an interpretation sentence; tarball verified (324 entries: `migrations/0.5.0.json`, `dist/primitives/` ×37, eight family files, `_properties.css`, `CHANGELOG.md`, no `.test.` files); CLI `diff`/`migrate` against the packed kit print the 0.5.0 slice and resolve the rules; `vp check` green once `apps/docs` typegen exists (`vp run docs#build`), core 190/190, cli 140/140, docs build passes. Maintainer runbook in `release.md` (core 0.5.0 → `core-v0.5.0`, CLI 0.3.0 → `cli-v0.3.0`, 0.4.2 skipped, CDN pins to repoint after publish, loose ends). ([35](issues/35-release-ready.md))
