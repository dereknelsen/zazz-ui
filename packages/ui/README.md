...
```js
import "@zazzdesign/ui/index.css"; // the stylesheet tree (cascade-ordered @imports)
...
Part of the [zazz-monorepo](https://github.com/dereknelsen/zazz-monorepo). `pnpm build` = tsc emit (in-place, next to each `.ts`) + `vp pack` bundles; `pnpm dev` = tsc watch. Authoring conventions: [CONVENTIONS.styles.md](./CONVENTIONS.styles.md) and [CONVENTIONS.scripts.md](./CONVENTIONS.scripts.md).
