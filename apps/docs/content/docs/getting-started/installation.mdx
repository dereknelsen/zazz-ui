...
```bash
pnpm add @zazz-ui/ui
# or: npm install @zazz-ui/ui
```

Import the stylesheet and optional component scripts:

```js
import "@zazz-ui/ui/index.css";
import "@zazz-ui/ui"; // component scripts
```

Pre-flattened bundles are available at `@zazz-ui/ui/zazz.css` and `@zazz-ui/ui/zazz.js`.
...
```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@zazz-ui/ui/src/ui/dialog/dialog.css" />
```

<Callout type="warn">
  Pin an explicit version in production (`@zazz-ui/ui@0.x.y`) to avoid unexpected changes.
</Callout>
...
pnpm dlx degit dereknelsen/zazz-ui/packages/ui/src ./zazz

# single component folder
pnpm dlx degit dereknelsen/zazz-ui/packages/ui/src/ui/dialog ./zazz/ui/dialog
```
