...
<Callout type="warn">
  Pin an explicit version in production (`@zazz-ui/core@0.x.y`) to avoid unexpected changes. When
  you move the pin across a breaking release, [Upgrading to 0.5](/docs/getting-started/upgrading)
  has the rename tables and the `zazz-ui migrate` command that applies them to your own files.
</Callout>
