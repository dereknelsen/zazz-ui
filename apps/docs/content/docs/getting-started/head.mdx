...
2. **Polyfill:** Interest Invokers (`invokers/interest`) — `interestfor` is Chromium-only. The Popover API and Invoker Commands are native across the supported browsers and are not polyfilled.
