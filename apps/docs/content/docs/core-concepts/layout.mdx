...
The Zazz container inverts this. It is not a box that constrains its contents. It is a grid of named width bands that spans the entire region, and every direct child chooses which band it occupies. The heading can sit in the `lg` band, the photo below it can bleed to the viewport edge, and the caption after that can return to `lg`, all as siblings in one flow, without touching the markup structure.
...
<section>
  <div class="container">
    <h2>Sits in the lg band (the default)</h2>
    <p>So does this paragraph.</p>
    <figure data-container="bleed">
      <img src="/wide.jpg" alt="" />
    </figure>
    <p>And the caption comes back to lg.</p>
  </div>
</section>
```
...
The band names are the breakpoint scale (`sm` 40rem to `2xl` 96rem), and `lg` is the default. The bands are fluid. Each one caps at its breakpoint width and centers while the viewport has room to spare, then fills the available space (minus gutters) once it doesn't. There is no snap at the breakpoint; the transition is continuous.

## The bands

| Band           | Width                            | Typical use                              |
| -------------- | -------------------------------- | ---------------------------------------- |
| `sm`           | caps at `--breakpoint-sm` 40rem  | narrow columns, short forms              |
| `md`           | caps at `--breakpoint-md` 48rem  | forms, focused content                   |
| `lg` (default) | caps at `--breakpoint-lg` 64rem  | standard page content                    |
| `xl`           | caps at `--breakpoint-xl` 80rem  | wide sections                            |
| `2xl`          | caps at `--breakpoint-2xl` 96rem | dashboards, galleries                    |
...
<section>
  <div class="container" data-container="xl">
    <h2>Everything in here defaults to xl</h2>
    <p>Including this.</p>
  </div>
</section>
```
...
<section>
  <div class="container">
    <h2>lg, the default</h2>
    <div class="grid @lg:grid-cols-3 gap-md" data-container="2xl">
...
- Mixed widths need no markup surgery. Text at `lg`, a table at `2xl`, a photo at `bleed`, all siblings.
...
- `@lg:container` acts as a container from `lg` up. Below that it renders as a normal full-width block.
- `@max-lg:container` is the inverse: a container only below `lg`. Above the breakpoint it falls back to whatever else the element carries.
...
```html
<section>
  <div class="@max-lg:container grid @lg:grid-cols-2 gap-lg">
    <!-- stacked in a band below lg, two columns above -->
  </div>
</section>
```
...
```css
:root {
  --gutters: var(--space-lg); /* edge padding reserved by every band, default --space-md */
  --article-lg: 65ch; /* reading measures for the article variant */
}
```

The `--breakpoint-*` tokens (40, 48, 64, 80, 96rem) set the band caps, but treat them as read-only. The `@md:` responsive utilities and the style-prop suffixes are gated by the `--bp-*` flags, which container queries set at literal lengths (CSS doesn't allow `var()` in a query prelude), so changing a breakpoint token would move the band widths while the responsive utilities keep switching at the old values.
