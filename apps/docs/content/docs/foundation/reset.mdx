...
Normalizes inline `<code>`, `<kbd>`, form inputs, search cancel buttons, and date/time pickers using token hooks (`--code-*`, `--ui-kbd-*`, `--placeholder-color`). Text selection uses primary-tinted highlights via `::selection`.
