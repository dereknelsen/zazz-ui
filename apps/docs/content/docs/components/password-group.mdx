...
`.password-group` provides an input group with a trailing ghost button that toggles password visibility. Wrap with `<ui-password>` to enhance with JavaScript while remaining fully functional as a masked text input without JS.
