...
Scripts in `@zazz-ui/ui` live alongside their components: shared runtime in `src/base/` and component logic in `src/ui/<name>/`. Authored in TypeScript, they compile to readable `.js` files that run as native ES modules in modern browsers without a required bundler.
