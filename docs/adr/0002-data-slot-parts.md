...
- Bare values (`data-slot="header"`) with descendant/`@scope` scoping: rejected because nested primitives make descendant selectors ambiguous, and `@scope` adds a support-policy dependency to solve a self-inflicted problem.
- Component-keyed attributes (`data-tooltip="content"`): rejected because bare keys collide with common consumer idioms (`data-tooltip="text"`), and prefixing them (`data-ui-tooltip`) creates ~35 verbose attribute names.
