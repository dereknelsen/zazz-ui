...
sync (idempotent by luck — one was in fact dead after the ui- rename), and the shared
...
an ancestor (`<ui-lightbox>` contains its dialog) and a descendant's holder (a carousel
...
- No `detail` payload — the dialog is the event target; add payload only with a use case.
