...
2. **Public `--ui-*` token, set inline**: when the value lands somewhere inline style
   cannot reach (`style="--ui-popover-inline-size: max-content"`).
3. **Raw inline style**: for a true same-element one-off
   (`style="max-inline-size: 16ch"`). Legitimate and supported.
...
arbitrary-value utility generation. That raised the question of what mechanism to support
...
through low-level variables.

## Decision

Adopt the ladder above and reject blanket hooks, based on two core reasons:
...
- **The cascade already solves the same-element case.** Inline `style` declarations beat
  ...
  zero capability over writing the property directly in `style=""`: it is pure overhead
  ...
  meter to progress geometry family). That is exactly where the existing `--ui-*` hooks
  already sit.
- **`--_` is private, and stays private.** `CONVENTIONS.styles.md` §5 draws a
  clear boundary: anything an app may set is a public `--ui-{component}-*` hook
  declared on `:root` in `@layer variables`; `--_*` is plumbing apps never touch, and it
  trends toward non-inheriting `@property` registration (a silent failure for hooks read
...
- Raw `style=""` for same-element one-offs is documented as legitimate: the cascade
  guarantees it is non-destructive to the kit layers.
- **CSP caveat**: under a strict `style-src` (no `unsafe-inline`/`unsafe-hashes`) the
  `style` attribute is blocked entirely, which affects rungs 2 and 3 equally (a custom
  property set inline is the same attribute). The CSP-safe rungs are 1 and 4. Hook
  variables were never a CSP workaround.
